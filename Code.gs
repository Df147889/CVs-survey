...
function doPost(e) {
  ...
  try {
    ...
    // ---- 3) تجهيز المؤهلات العلمية في خانة واحدة ----
    var QUAL_AR = { Diploma: 'دبلوم', Bachelor: 'بكالوريوس', Master: 'ماجستير', PhD: 'دكتوراه' };
    var educationsText = '';
    if (data.educations && data.educations.length) {
      educationsText = data.educations.map(function (e, i) {
        return 'مؤهل ' + (i + 1) + ':\n' +
               '• الدرجة: ' + (QUAL_AR[e.qualification] || e.qualification || '') + '\n' +
               '• الجامعة: ' + (e.university || '') + '\n' +
               '• التخصص: ' + (e.specialization || '');
      }).join('\n———————————————\n');
    }

    // ---- 4) تجهيز المشاريع في خانة واحدة منسّقة ----
    var projectsText = '';
    ...
    // ---- 5) تجهيز الخبرات السابقة في خانة واحدة ----
    var prevExpText = '';
    if (data.previousExperiences && data.previousExperiences.length) {
      prevExpText = data.previousExperiences.map(function (x, i) {
        return 'خبرة ' + (i + 1) + ':\n' +
               '• المسمى: ' + (x.title || '') + '\n' +
               '• الوصف: ' + (x.description || '');
      }).join('\n———————————————\n');
    ...
    sheet.appendRow([
      ...
      educationsText,                           // المؤهلات العلمية
...
function getSheet_() {
  ...
  if (sheet.getLastRow() === 0) {
    sheet.appendRow([
      ...
      'المؤهلات العلمية', 'الشهادات المهنية', 'اللغات',
